...
const EMPTY = {
	name: "",
	description: "",
	brand: [] as string[], // <-- "as string[]" qo'shildi
	type: "valves_in_out",
	image: "",
};
				...
				<section className="admin-card">
					...
					<form className="admin-form" onSubmit={handleSubmit}>
						...
						<div className="field-row">
							<div className="field-group">
								<label className="field-label">Brend (bir nechtasini tanlash mumkin)</label>
										...
										<label key={b.value} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
											<input
												...
												onChange={(e) => {
													...
													setForm(prev => {
														// Текущий список брендов в форме
														const currentBrands = prev.brand as string[];

														return {
															...prev,
															brand: isChecked
																? [...currentBrands, b.value] // Добавляем бренд
																: currentBrands.filter(item => item !== b.value) // Удаляем бренд
														};
													});
												}}
											/>
								...
								<div key={p.id} className={`admin-product${editingId === p.id ? " is-editing" : ""}`}>
									...
									<div className="product-info">
										...
										<span className="product-meta">
											{Array.isArray(p.brand) ? p.brand.map(b => BRANDS.find(brand => brand.value === b)?.label || b).join(', ') : p.brand} · {p.type}
										</span>
									</div>
